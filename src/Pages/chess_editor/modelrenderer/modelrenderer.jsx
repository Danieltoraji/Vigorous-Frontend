    ...
    const renderBaseShape = () => {
        ...
        switch (type) {
            ...
            case 'special': // 异形类型
                ...
                // 调试：检查异形数据
                console.log('异形底座数据:', {
                    hasCustomShape: !!base.customShape,
                    profilePointsCount: baseCustomShape.profilePoints?.length || 0,
                    pathPointsCount: baseCustomShape.pathPoints?.length || 0,
                    profilePoints: baseCustomShape.profilePoints,
                    pathPoints: baseCustomShape.pathPoints
                });
                
                // 检查数据是否有效
                if (!baseCustomShape.profilePoints || baseCustomShape.profilePoints.length < 3) {
                    console.warn('异形底座轮廓点数不足 (< 3)，无法渲染');
                    return null; // 或者显示一个错误提示模型
                }
    ...
    const renderColumnShape = () => {
        ...
        switch (type) {
            ...
            case 'special': // 异形类型
                ...
                // 调试：检查异形数据
                console.log('异形柱体数据:', {
                    hasCustomShape: !!column.customShape,
                    profilePointsCount: columnCustomShape.profilePoints?.length || 0,
                    pathPointsCount: columnCustomShape.pathPoints?.length || 0,
                    profilePoints: columnCustomShape.profilePoints,
                    pathPoints: columnCustomShape.pathPoints
                });
                
                // 检查数据是否有效
                if (!columnCustomShape.profilePoints || columnCustomShape.profilePoints.length < 3) {
                    console.warn('异形柱体轮廓点数不足 (< 3)，无法渲染');
                    return null;
                }
